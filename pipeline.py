...
# Kubernetes will inject these when it runs the pod
...
# Standard Boto3 client for uploading files
...
def extract():
    ...
    # Save raw data directly to MinIO 'raw-data' bucket
...
def preprocess():
    ...
    # Save processed data directly back to MinIO
...
def train_and_evaluate():
    print("--- Step 3: Training & Evaluating SVM ---")
    ...
    # --- Save plots to memory buffer, then upload to MinIO ---
    ...
    img_buffer.seek(0)
    s3_client.put_object(Bucket='models', Key='confusion_matrix.png', Body=img_buffer)
    plt.clf()
    ...
    plt.savefig(img_buffer, format='png')
    img_buffer.seek(0)
    s3_client.put_object(Bucket='models', Key='roc_curve.png', Body=img_buffer)
    
    # 3. Save the model object
    model_buffer = io.BytesIO()
    joblib.dump(model, model_buffer)
    model_buffer.seek(0)
    s3_client.put_object(Bucket='models', Key='svm_model.pkl', Body=model_buffer)
    
    print("Training complete. Model and images uploaded to 'models' bucket.")
